...
{  
    HRESULT hr = S_OK;
    if (m_ownerFilter->CanStopListenerThread())
        m_ownerFilter->StopListenerThread();

    if (!m_ownerFilter->IsListenerThreadStarted())
    {
        if (FILL_ZERO_DATA == m_fillState)
        {
            intrusive_ptr<IPin> inputPin;
            hr = m_pInputPin->QueryInterface(
    ...
    TRACE(L"m_nextSampleBeginTime: %I64d\n", m_nextSampleBeginTime);
    return hr;
}
...
{
    CAutoLock lock(m_pLock);
    int needSendPackets = m_stateInfo->currentNeedSendPackets;
    if (needSendPackets <= 0)
        return S_FALSE;
